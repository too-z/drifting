"""Improved precision/recall (Kynkaanniemi et al.). Port of utils/jax_fid/precision_recall.py.

Identical structure and batching to the JAX version; the only change is that
the jitted jnp pairwise-distance kernel is replaced with torch on CPU.

Semantic delta: the JAX version requested float64 but silently ran in float32
unless ``jax_enable_x64`` was set; this port computes distances in true
float64. Membership tests are thresholded, so results only differ if a
distance ties a radius at float32 resolution.
"""
...
class DistanceBlock:
    """
    Calculate pairwise distances between vectors using torch (CPU, float64).
    ...
    def pairwise_distances(self, U, V):
        ...
        """
        U_t = torch.as_tensor(np.asarray(U), dtype=torch.float64)
        V_t = torch.as_tensor(np.asarray(V), dtype=torch.float64)

        distances = _batch_pairwise_distances(U_t, V_t)
    ...
    def less_thans(self, batch_1, radii_1, batch_2, radii_2):
        ...
        """
        batch_1_t = torch.as_tensor(np.asarray(batch_1), dtype=torch.float64)
        batch_2_t = torch.as_tensor(np.asarray(batch_2), dtype=torch.float64)
        radii_1_t = torch.as_tensor(np.asarray(radii_1), dtype=torch.float64)
        radii_2_t = torch.as_tensor(np.asarray(radii_2), dtype=torch.float64)

        # Compute pairwise distances
        distances = _batch_pairwise_distances(batch_1_t, batch_2_t)

        # Check if distances are within radii
        # batch_1_in: for each point in batch_1, check if it's within any radius in radii_2
        batch_1_in = torch.any(distances[..., None] <= radii_2_t, dim=1)

        # batch_2_in: for each point in batch_2, check if it's within any radius in radii_1
        batch_2_in = torch.any(distances[..., None] <= radii_1_t[:, None], dim=0)

        return batch_1_in.numpy(), batch_2_in.numpy()
...
def _batch_pairwise_distances(U, V):
    ...
    norm_u = torch.sum(torch.square(U), dim=1)
    norm_v = torch.sum(torch.square(V), dim=1)
